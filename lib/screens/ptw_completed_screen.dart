...
class PtwCompletedScreen extends StatelessWidget {
  const PtwCompletedScreen({super.key});

  @override
  Widget build(BuildContext context) {
    final PtwCompletedController controller = Get.find<PtwCompletedController>();
    // Initialize the controller specifically for completion checklist
    final ChecklistController checklistController = Get.put(
      ChecklistController(ChecklistType.ptwCanceletionCompletion),
      tag: ChecklistType.ptwCanceletionCompletion.toString(), // Unique tag
    );
    ...
    return Scaffold(
      ...
      body: MainLayout(
        ...
        child: Obx(() {
          ...
          return ListView(
            padding:
                const EdgeInsets.only(left: 16, right: 16, top: 0, bottom: 50),
            children: [
              // Checklist Section
              ListView.builder(
                shrinkWrap: true,
              ...
              Obx(() => BottomNavigationButtons(
                    onBackPressed: controller.isLoading.value ||
                            checklistController.isSubmitting.value
                        ? null
                        : () => Get.back(),
                        ...
                        : () async {
                            if (ptwId != null) {
                              ...
                              controller.submitPtwCompletion(
                                ...
                                checklistItems: checklistController
                                    .checklistItems, // ✅ FIXED
                              );
                            }
                          },
                    nextText: 'Submit',
  ...
  Widget _buildAttachmentsSection(PtwCompletedController controller) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text(
          'After Work Picture /کام کے بعد کی تصویر',
          style: TextStyle(
              fontSize: 18,
  ...
  Widget _buildImagePreviews(PtwCompletedController controller) {
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        return _buildImagePreview(
            controller.images[index], () => controller.removeImage(controller.images[index]));
      },
    );
  }
        ...
        Positioned(
          ...
          child: InkWell(
            ...
            child: const CircleAvatar(
              radius: 12,
              backgroundColor: Color(0xFF002997),
